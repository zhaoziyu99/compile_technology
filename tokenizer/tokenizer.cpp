		...
		while (true) {
			...
			case UNSIGNED_INTEGER_STATE: {
				...
				if (!current_char.has_value()) {
					...
					for (int i = 0; i < len; i++)
					{
						if(!miniplc0::isdigit(str[i]))//解析失败,含有非数字字符,返回编译错误
							return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos, ErrorCode::ErrInvalidInput));
					}
					ss.clear();
					ss.str(str);
			...
			case IDENTIFIER_STATE: {
				...
				//解析成功则返回无符号标识符类型的token，否则返回编译错误
				if (!current_char.has_value()) {
					string str;
					ss >> str;
					int len = str.length();
					if(!miniplc0::isalpha(str[0]))
						return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos, ErrorCode::ErrInvalidInput));
					for (int i = 1; i < len; i++)
					{
						if (!miniplc0::isalpha(str[i]) || !miniplc0::isdigit(str[i]))//解析失败,含有非数字字符,返回编译错误
							return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos, ErrorCode::ErrInvalidInput));
					}
